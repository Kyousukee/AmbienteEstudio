	angular.module('AngularJSUnidad', [])
	.controller('AngularUnidadController', ['$scope', '$http', function($scope, $http) {
        ...
        self.cargarCursos = function(){
			...
			$http({
				method: 'POST',
				url: '../Controlador/MantenedorUnidades.php',
				data: form_data,
        ...
        /*INICIO LISTADO DE PROVEEDORES*/
		 ...
		 $scope.LstPAsig = self.ListadoUnidades;

		 self.ListadoUnidades();
		...
		self.ProfeFormIng = function() {
			...
			'&btn_grabar=0';*/
			var form_data = 'cbocurso='+self.cbocursos+'&txtdescripcion='+self.txtdescripcion+'&cboestado='+self.cboestado+'&txtasig='+self.cboasignatura+'&txtnumero='+self.txtnunidad+
			'&btn_grabar=0';
		...
		self.proveedorFormEdi = function(IdUni){
			var form_data ="btn_editar=0&IdUni="+IdUni+"";
			$http({
			  method: 'POST',
			  url: '../Controlador/MantenedorUnidades.php',
			  data: form_data,
			...
			.success(function(data) {
				console.log(data);
				$scope.angCtrl.txtidunidad  =data[0].id_uni;
				$scope.angCtrl.cbocursos=data[0].id_dcurso;
				self.cargarasignatura2();

				$scope.angCtrl.cboasignatura=data[0].id_asig;
				$scope.angCtrl.txtnunidad  =parseInt(data[0].num_uni, 10);	        
				$scope.angCtrl.txtdescripcion  =data[0].desc_uni;	        
		        
			    $scope.angCtrl.cboestado       =data[0].Est_uni;
		...
		self.proveedorFormEli = function(iduni) {

			var form_data ='btn_actdes=0&iduni='+iduni;
			$http({
					  method: 'POST',
					  url: '../Controlador/MantenedorUnidades.php',
					  data: form_data,
