	angular.module('AngularJSAlumno', [])
	.controller('AngularAlumnoController', ['$scope', '$http', function($scope, $http) {
        ...
        /*INICIO LISTADO DE PROVEEDORES*/
		 ...
		 self.ListadoAlumnos = function(){
		 var form_data ="btn_listar=0&txtbuscar="+self.txtbuscar+"";
		 $http({
		   method: 'POST',
		   url: '../Controlador/MantenedorAlumno.php',
		   data: form_data,
		 ...
		 $scope.LstPEst = self.ListadoAlumnos;
		...
		self.ProfeFormIng = function() {
			...
			'&txtfono='+self.txtfono+'&txtemail='+self.txtemail+'&cbodcurso='+self.cbocursos+'&cboletrcurso='+self.letracursos+
			'&btn_grabar=0';
		...
		self.proveedorFormEli = function(IdEst,RutEst) {

			var form_data ='btn_actdes=0&IdEst='+IdEst+'&RutEst='+RutEst;
			$http({
					  method: 'POST',
					  url: '../Controlador/MantenedorAlumno.php',
					  data: form_data,
